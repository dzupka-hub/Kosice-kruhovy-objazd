...
// (prestavana na kruhovy objazd) - 6 vztahov, porovnatelnych s meranim 12/2024.
//
// Uzly:  A = Kostolianska cesta 694/39 (smer Kostolany)
//        B = Kostolianska cesta 32     (smer Centrum)  - overene kalibraciou
...
const UZLY = {
  ...
  B: { lat: 48.741252, lon: 21.247154, nazov: "Centrum" },
  C: { lat: 48.743326, lon: 21.246089, nazov: "pod Hradovou" },
  D: { lat: 48.742402, lon: 21.252225, nazov: "Vodárenská" },
};

// Merane vztahy + dlzka z merania 2024 (kontrola, ze meriame to iste)
const VZTAHY = [
...
(async () => {
  ...
  for (const v of VZTAHY) {
    try {
      ...
      console.log(
        ...
        `(volne ${r.volny} s, zdrzanie ${r.cas - r.volny} s, ${r.dlzka} m)${znak}`
      );
    } catch (e) {
  ...
  const riadok = [
    ...
    const r = entry[v.id];
    return (!r || r.error) ? ["", "", "", ""]
      : [r.cas, r.volny, r.cas - r.volny, r.dlzka];
  })).join(";");
